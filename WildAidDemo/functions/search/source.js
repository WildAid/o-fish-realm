exports = function(query){
    var boardingsCollection = context.services.get("mongodb-atlas").db("wildaid").collection("BoardingReports");
    let vessels = [];
    let vesselsAmount = [];
    let crew = [];
    let boardings = [];
    ...
    if (query != "") {
        vessels = boardingsCollection.aggregate([
            {
                '$search': {
                    'text': {
                        'query': query, 
        ...
        vesselsAmount = boardingsCollection.aggregate([
            {
                '$search': {
                    'text': {
                        'query': query, 
        ...
        crew = boardingsCollection.aggregate([
            {
                '$search': {
                    'term': {
                        'query': query, 
        ...
        boardingsAmount = boardingsCollection.aggregate([
            {
                '$search': {
                    'text': {
                        'query': query, 
        ...
        highlighted = boardingsCollection.aggregate([
            {
                '$search': {
                    'term': {
                        'query': query, 
